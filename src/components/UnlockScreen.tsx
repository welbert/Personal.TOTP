...
export default function UnlockScreen({ onUnlocked, onReset }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-theme-bg px-8 fade-in">
      <div className="w-12 h-12 rounded-2xl bg-theme-raised flex items-center justify-center mb-5">
        <LockIcon className="w-6 h-6 text-theme-3" />
      </div>

      <h1 className="text-lg font-semibold text-theme-1 mb-1">{t("unlock.title")}</h1>
      <p className="text-sm text-theme-3 mb-6">{t("unlock.subtitle")}</p>
      ...
      <button
        ...
        className="mt-6 text-xs text-theme-5 hover:text-theme-3 transition-colors"
      >
        {t("reset.link")}
      </button>
